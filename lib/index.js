...
var student_data_cn = "https://schale.gg/data/cn/students.json";
var student_data_jp = "https://schale.gg/data/jp/students.json";
var student_data_tw = "https://schale.gg/data/tw/students.json";
var student_data_kr = "https://schale.gg/data/kr/students.json";
var student_data_zh = "https://schale.gg/data/zh/students.json";
var FMPS = class {
  static {
    __name(this, "FMPS");
  }
  ...
  async img_to_channel(data, appId, secret, channelId) {
    ...
    payload.append("file_image", new Blob([data.buffer], { type: "image/png" }), "image.jpg");
...
var guideConfig = import_koishi3.Schema.intersect([
  import_koishi3.Schema.object({
    ...
    map: import_koishi3.Schema.number().default(30).description("当前日服最高主线地图")
  }).description("攻略系统设置")
]);
var synonyms = {
...
var guide_systeam = {
  async apply(ctx3, config) {
    const maxmap_sms2 = config.guide.map;
...
`;
    ...
    ctx3.command("攻略/国服千里眼").action(async ({ session }) => {
      ...
      if (arodatas.code == 200) {
        await fmp2.guide_download_image(root_guide, arona_cdn + "/s" + arodatas.data[0].content, arodatas.data[0].hash, log_on);
        return import_koishi3.h.image((0, import_url.pathToFileURL)((0, import_path.resolve)(root_guide + "/d7eb993e9be07a74f0aca5a854dbe963.png")).href);
      } else {
        return session.text(".error");
      }
    });
...
async function gacha_f(ctx3, config) {
  ...
  async function get_gacha_stu() {
    const wiki_data = await ctx3.http.get(`https://ba.gamekee.com/v1/cardPool/query-list?status=1`, {
      headers: {
        "Game-Alias": "ba"
      }
    });
    ...
    for (let i = 0; i < wiki_data.data.length; i++) {
      ...
      if (wiki_data.data[i].server_id == 16) {
        ...
        pick_cn_time.push(fmp2.formatTimestamp(wiki_data.data[i].start_at));
        ...
        pick_jp_time.push(fmp2.formatTimestamp(wiki_data.data[i].start_at));
        ...
        pick_in_time.push(fmp2.formatTimestamp(wiki_data.data[i].start_at));
...
${i2}国服十连 爱丽丝
  ...
  ctx3.command("抽漫画", "随机抽取ba官方漫画").action(async ({ session }) => {
    try {
      const ii = random2.int(0, manga_jsondata.length);
      console.log(ii);
      console.log(manga_jsondata[ii].wikiurl);
      const manga = await ctx3.http.get(manga_jsondata[ii].wikiurl, {
        headers: {
          Referer: "https://www.gamekee.com/"
        },
        responseType: "arraybuffer"
      });
      const manga_buff = Buffer.from(manga);
      const channe_url = await fmp2.img_to_channel(manga_buff, session.bot.config.id, session.bot.config.secret, qqguild_id);
      const wh = await fmp2.get_wi_hi(manga_buff);
...
async function cal_favorable(ctx3, config) {
  ...
  async function local_update() {
    async function get_stu_favo() {
      let in_json_create_data = [];
      try {
        const dbdata = await ctx3.http.get("https://schale.gg/data/cn/students.json");
        for (let i = 0; i < dbdata.length; i++) {
...
var plugin_Config = import_koishi9.Schema.intersect([
  import_koishi9.Schema.object({
    ...
    auto_update: import_koishi9.Schema.boolean().default(true).experimental().description("是否每次重载都下载资源 **建议关闭**"),
    inte: import_koishi9.Schema.intersect([
      import_koishi9.Schema.object({
        type: import_koishi9.Schema.union(["模板md", "原生md"]).required()
      }).description("QQ markdown设置"),
      import_koishi9.Schema.union([
        import_koishi9.Schema.object({
          type: import_koishi9.Schema.const("百度审核").required(),
          id: import_koishi9.Schema.string().description("APP ID"),
          APIKey: import_koishi9.Schema.string().description("API Key").role("secret"),
          SKey: import_koishi9.Schema.string().description("Secret Key").role("secret")
        }).description("模板md"),
        import_koishi9.Schema.object({
          type: import_koishi9.Schema.const("原生md").required(),
          callback_butt: import_koishi9.Schema.boolean().description("是否使用回调按钮，开启后一些按钮将会替换成回调按钮，该权限需要邮件给qq开放平台申请")
        }).description("原生md")
      ])
    ])
  }).description("插件基础设置")
