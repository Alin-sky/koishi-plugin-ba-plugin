...
exports.draw_config = koishi_1.Schema.intersect([
    koishi_1.Schema.object({
        modle: koishi_1.Schema.boolean().required().description('选择canvas渲染模式（开：canvas/关：puppeteer）'),
    }).description('渲染模式设置'),
    koishi_1.Schema.object({
...
async function cal_favorable(ctx, config) {
    ...
    async function cre_favor_list(gifts, characters) {
        let json = [];
    ...
    const liwu = await fmp.json_parse(`${root_json}/liwu.json`);
    const characters = await fmp.json_parse(`${root_json}/favor.json`);
    await get_stu_favo();
