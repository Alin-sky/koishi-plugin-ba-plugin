...
exports.plugin_ass = exports.inject = void 0;
...
async function cal_favorable(ctx, config) {
    ...
    async function local_update() {
        ...
        async function cre_favor_list() {
            const gifts = await fmp.json_parse(`${root_json}/liwu_list_rep.json`);
            const characters = await fmp.json_parse(`${root_json}/favor_stu_tap.json`);
            let json = [];
    ...
    ctx.command("好感计算 <arg1> <arg2>", "好感度需求计算器")
        ...
        if (!arg1) {
            ...
            try {
                let favor_lev = (0, favorability_1.getFavorLv)(arg1, session);
                let faovr;
            ...
            try {
                ...
                if (/^\d+$/.test(arg2[0])) {
                    innum = arg1 + "-" + arg2[0];
                    let favor_lev = (0, favorability_1.getFavorLv)(innum, session);
                    let faovr;
                    if (typeof favor_lev === "string") {
                        return favor_lev + "\n" + help_text;
                    }
