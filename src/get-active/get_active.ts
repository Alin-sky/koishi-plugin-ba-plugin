...
export async function active_get(ctx: Context, config: Config) {
    ...
    async function draw_active(wiki_data) {
        ...
        for (let i = 0; i < wiki_data.data.length; i++) {
            if (!(wiki_data.data[i].picture == '')) {
                ...
                const m = 260 / acvimg[i - s]['naturalHeight']
                const hei = acvimg[i - s]['naturalHeight'] * m
                jud_time(wiki_data.data[i].begin_at) ? yls += (hei + 160) : yrs += (hei + 160)
            } else {
                jud_time(wiki_data.data[i].begin_at) ? yls += 110 : yrs += 110
                s++
            }
        }

        let height; yls > yrs ? height = yls : height = yrs
        const canvas = await ctx.canvas.createCanvas(1600, height + 350);
        const c = canvas.getContext('2d');
        c.fillStyle = '#FFFFFF';
        c.fillRect(0, 0, 1600, height + 400);
        let x = 30, rad = 15, y = 30, wid = 700, hei = 150, color = '', yl = 230, yr = 230
        ...
        c.fillText('进行中', x + 50, y + 90)
        c.fillStyle = '#000000';
        c.font = `bold 40px Arial`;
        c.fillText('当前时间：', x + 300, y + 50)
        c.fillText(new Date().toLocaleString(), x + 300, y + 100)
        let xr = 800
        c.beginPath();
        c.moveTo(xr + rad, y);
        ...
        for (let i = 0; i < wiki_data.data.length; i++) {
            ...
            switch (swin) {
                case '日服true': color = '#EBDBFF'; x = 30; y = yl; break
                case '日服false': color = '#BCA0CC'; x = 800; y = yr; break
                case '国际服true': color = '#D4ECFF'; x = 30; y = yl; break
                case '国际服false': color = '#9AAECC'; x = 800; y = yr; break
                case '国服true': color = '#FFD8D8'; x = 30; y = yl; break
                case '国服false': color = '#D4A2A2'; x = 800; y = yr; break
            }
            if (!(wiki_data.data[i].picture == '')) {
                const m = 250 / acvimg[i - ss]['naturalHeight']
                const hei = acvimg[i - ss]['naturalHeight'] * m
                const widimg = acvimg[i - ss]['naturalWidth'] * m
                const move = 150
                c.beginPath();
                c.moveTo(x + rad, y);
                ...
                if (wiki_data.data[i].title.length > 32) {
                    ypush = 20
                    y += ypush
                    c.drawImage(acvimg[i - ss], x + 30, y + 50, widimg, hei)
                    y -= ypush
                } else {
                    c.drawImage(acvimg[i - ss], x + 30, y + 50, widimg, hei)
                }
                ...
                c.fillText('开始时间：' + timetemp[0], x + 50, y + hei + 80 + ypush)
                c.fillStyle = '#420000';
                c.fillText('结束时间：' + timetemp[1], x + 50, y + hei + 110 + ypush)

                c.fillStyle = '#000000';
                c.font = `bold 30px Arial`;
                c.fillText(timetemp[2], x + 450, y + hei + 90)
                ...
                c.fillText(timetemp[3], x + 450, y + hei + 120)
                ...
                c.fillStyle = '#001D42';
                c.font = `bold 21px Arial`;
                c.fillText('开始时间：' + timetemp[0], x + 50, y + 90)
                c.fillStyle = '#420000';
                c.fillText('结束时间：' + timetemp[1], x + 50, y + 120)

                c.fillStyle = '#000000';
                c.font = `bold 30px Arial`;
                c.fillText(timetemp[2], x + 450, y + 90)
                ...
                c.fillText(timetemp[3], x + 450, y + 120)
            ...
            jud_time(wiki_data.data[i].begin_at) ? yl += 160 : yr += 160
    ...
    async function createCachedImageGenerator(wiki_data): Promise<() => Promise<string>> {
        // 如果已经有缓存函数实例，直接返回它
        if (cachedImageGeneratorInstance) {
            return cachedImageGeneratorInstance;
        }
        ...
        // 存储新的缓存函数实例
        cachedImageGeneratorInstance = cachedImageGenerator;
    ...
    ctx.command('活动日程', 'ba活动查询')
        ...
        .action(async () => {
            ...
            return h.image(await cachedGenerator)
        })
