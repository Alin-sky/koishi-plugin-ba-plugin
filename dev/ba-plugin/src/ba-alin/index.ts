      ...
      .action(async ({ session }, ...args) => {
        ...
        if ((args[0]) == null) {
          ...
          if ((args[0]) === '千里眼' || (args[0]) === '国际服千里眼') {
            return (h('image', { url: "https://cdnimg.gamekee.com/wiki2.0/images/w_2580/h_5568/829/43637/2023/8/7/103388.png" }))
          }
          if ((args[0]) === '国服千里眼') {
          ...
          if (outstu.status == 200) {
            ...
            //模糊匹配撤回
            setTimeout(() => { session.bot.deleteMessage(session.channelId, Message[0]) }, time1)
            let timeout = 20000//20s
            let obj = await session.prompt(timeout)
            if (!obj) {

              const message1 = await session.send(h('at', { id: session.userId })
                + '呜呜，等待超时，请重新触发指令')
              setTimeout(() => { session.bot.deleteMessage(session.channelId, message1[0]) }, time1)
              ...
              await downloadImage(url, filename).catch(console.error);//下载
