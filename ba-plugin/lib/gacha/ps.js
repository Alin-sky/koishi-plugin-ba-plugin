...
(function (gachaImage) {
    ...
    async function stuCardMaker(stat, cardtemp, RPool, SRPool) {
        let CARDSTAR;
        let CARDSTAR2;
        if (RPool.some((item) => item.name === cardtemp.name)) {
            CARDSTAR = ba_alin_1.alincloud + 'stuimg' + '/assets/☆card.png';
            CARDSTAR2 = ba_alin_1.alincloud + 'stuimg' + '/assets/card☆1.png';
        }
        else if (SRPool.some((item) => item.name === cardtemp.name)) {
            CARDSTAR = ba_alin_1.alincloud + 'stuimg' + '/assets/☆☆card.png';
            CARDSTAR2 = ba_alin_1.alincloud + 'stuimg' + '/assets/card☆2.png';
        }
        else {
            CARDSTAR = ba_alin_1.alincloud + 'stuimg' + '/assets/☆☆☆card.png';
            CARDSTAR2 = ba_alin_1.alincloud + 'stuimg' + '/assets/card☆3.png';
        }
    ...
    async function headMaker(stat, cardStar, cardStar2, cardtemp) {
        let stuImage;
        const MARGIN = ba_alin_1.alincloud + 'stuimg' + '/assets/margin.png';
        if (stat == -1) {
            stuImage = await jimp_1.default.read(cardtemp.avatar);
        }
        else {
            const STUIMAGEURL = ba_alin_1.alincloud + 'stuimg' + '/assets/Student/' + cardtemp.name + '.png';
        ...
        TEMPIMAGE.writeAsync(path_1.default.resolve(__dirname, '../assets/temp/' + cardtemp.name + '.png'));
        return new Map().set('buffer', (await TEMPIMAGE.getBufferAsync(jimp_1.default.MIME_PNG)));
    }
    //位图处理
    ...
    async function setPickUp(stuCard, font) {
        return new Map().set('buffer', (await stuCard.print(font, 15, -10, "Pick Up!").getBufferAsync(jimp_1.default.MIME_PNG)));
    }
    //结算图
    async function result(cardArray, stat, RPool, SRPool) {
        const GACHABG = ba_alin_1.alincloud + 'stuimg' + '/assets/gachaBG.png';
        const TEMPCARD = ba_alin_1.alincloud + 'stuimg' + '/assets/☆card.png';
        ...
        let stuPromises = cardArray.map(async (cardtemp) => {
            if (stat === -1) {
                return await jimp_1.default.read((await stuCardMaker(stat, cardtemp, RPool, SRPool)).get('buffer'));
            }
            else {
                let stuCard;
                if (fs_1.default.existsSync(path_1.default.resolve(__dirname, '../assets/temp/' + cardtemp.name + '.png'))) {
                    stuCard = await jimp_1.default.read(path_1.default.resolve(__dirname, '../assets/temp/' + cardtemp.name + '.png'));
                }
                else {
                    stuCard = await jimp_1.default.read((await stuCardMaker(stat, cardtemp, RPool, SRPool)).get('buffer'));
                }
                if (cardtemp.pickup) {
                    return await jimp_1.default.read((await setPickUp(stuCard, FONT)).get('buffer'));
                }
                else {
                    return await jimp_1.default.read(path_1.default.resolve(__dirname, '../assets/temp/' + cardtemp.name + '.png'));
                }
            }
        });
        ...
        return new Map().set('buffer', (await gachaBG.print(FONT2, gachaBG.getWidth() - 250, gachaBG.getHeight() - 140, stat).getBase64Async(jimp_1.default.MIME_PNG)));
    }
    gachaImage.result = result;
    //获取url图片
...
})(gachaImage || (exports.gachaImage = gachaImage = {}));
