...
exports.gachaplugin = ({
    apply(ctx, config) {
        ctx.on('ready', async () => {
            ...
            database_1.DB.stuTable(ctx);
            database_1.DB.stuUpdate(ctx);
            database_1.DB.BAUserTable(ctx);
        });
        ctx.command("ba", '抽卡模拟器').example("ba 日服 十连")
            ...
            session.send((0, koishi_1.h)('message', [session.author.username + '的抽卡结果：\n', (0, koishi_1.h)('image', { url: gacha_1.path }),
                koishi_1.h.image(result.get('buffer3'), 'image/png')]));
        });
