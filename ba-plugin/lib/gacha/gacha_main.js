...
exports.gacha_f = exports.using = exports.inject = void 0;
...
//Alin’s ba-gacha-systems v3 2024-04-18
...
async function gacha_f(ctx, config) {
    ...
    */
    async function creat_img(stu_gacha, print, serverid) {
        const image = await ctx.canvas.loadImage(`file://${root_img}/background.png`);
        const canvas = await ctx.canvas.createCanvas(2048, 1200);
        const back = [
            await ctx.canvas.loadImage(`file://${root_img}/1sta.png`),
            await ctx.canvas.loadImage(`file://${root_img}/1sta.png`),
            await ctx.canvas.loadImage(`file://${root_img}/2sta.png`),
            await ctx.canvas.loadImage(`file://${root_img}/3sta.png`),
        ];
        ...
        let move = 106;
        let z = 42;
        const printimg = await ctx.canvas.loadImage(`file://${root_img}/print_${serverid}.png`);
        for (let i = 0; i < 10; i++) {
            const image = await ctx.canvas.loadImage(`file://${root_img}/${stu_gacha[0][i]}_g.png`);
            const pick = await ctx.canvas.loadImage(`file://${root_img}/pickup.png`);
            ctximg.save();
    ...
    async function draw_200_img(stu_gacha) {
        ...
        const back = [
            await ctx.canvas.loadImage(`file://${root_img}/1sta.png`),
            await ctx.canvas.loadImage(`file://${root_img}/1sta.png`),
            await ctx.canvas.loadImage(`file://${root_img}/2sta_1.png`),
            await ctx.canvas.loadImage(`file://${root_img}/3sta_1.png`),
        ];
        ...
        async function draw_char(id, star, x, y) {
            ...
            let move = 109;
            let z = 42;
            const stu_img = await ctx.canvas.loadImage(`file://${root_img}/${id}_g.png`);
            c.save();
            if (star == 4) {
...
${i2}国服十连 爱丽丝
    ...
    ctx.command("ba抽卡/日服十连 <message:text>")
        ...
        if (!message) {
            await session.send('正在抽取日服常驻池子，请老师稍等哦');
    ...
    ctx.command('ba抽卡/国际服一井 <message:text>')
        ...
        if (!message) {
            ...
            if (stu_server_jud(id) < server_ids) {
                ...
                const muzhu = cal_muzhu(stu_gacha);
                if (session.event.platform == 'qq' && mdswitch) {
                    const md = markdown_gacha_sub(session, server_ids, muzhu);
                    await session.qq.sendMessage(session.channelId, md);
                }
                else {
                    await session.send("总共获得神明精髓：" + muzhu + "个");
