...
function 日服up() {
    //删除up角
    for (var i = 0; i < cardPool.oneStar.length; i++) {
        if (cardPool.oneStar[i].name === index_1.jaup) {
            cardPool.threeStar = cardPool.threeStar.filter(card => card.name !== index_1.jaup);
        }
    }
    //主体
    function drawCards() {
...
function 国际up() {
    //删除up角
    //删除日服角色
    cardPool.threeStar = cardPool.threeStar.filter(card => card.name !== "叶渚"
        && card.name !== "南"
        && card.name !== "琉美"
        && card.name !== "果穗"
        && card.name !== "佳代子（正月）"
        ...
        && card.name !== "遥香（正月）");
    for (var i = 0; i < cardPool.oneStar.length; i++) {
        if (cardPool.oneStar[i].name === index_1.inup) {
            cardPool.threeStar = cardPool.threeStar.filter(card => card.name !== index_1.inup);
        }
    }
    //主体
    function drawCards() {
