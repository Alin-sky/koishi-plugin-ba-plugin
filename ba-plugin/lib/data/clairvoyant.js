...
exports.getMessage = exports.accumulate = void 0;
// 卡池类，用于记录卡池的时间以及up角色（没用上，想多了）
class GachaPool {
    constructor(name, startTime, endTime, up1, up2, up3, up4, up5) {
        this.name = name;
...
function accumulate(endTime, check, jjc, ability, rank) {
...
function getMessage(input) {
    const timeTag = /(\d{4}\/\d{1,2}\/\d{1,2})/;
    const timeResult = input.match(timeTag);
    const endTime = timeResult && timeResult[1];
    const checkIndex = input.indexOf("月卡");
    let check = false;
    if (checkIndex !== -1 && checkIndex + 2 < input.length) {
        let a = input.charAt(checkIndex + 2);
        if (a === "无") {
            check = false;
        }
        else if (a === "有") {
    ...
    const jjcTag = /jjc(\d+)/;
    const jjcResult = input.match(jjcTag);
    const jjc = jjcResult && jjcResult[1];
    const challengeTag = /挑战(\d+)/;
    const challengeResult = input.match(challengeTag);
    const challenge = challengeResult && challengeResult[1];
    const rankIndex = input.indexOf("总力");
    let rank = "";
    if (rankIndex !== -1 && rankIndex + 2 < input.length) {
        let a = input.charAt(rankIndex + 2);
        if (a === "1" || a === "一") {
            rank = "1";
        }
        else if (a === "2" || a === "二") {
    ...
    return [endTime, check, parseInt(jjc), parseInt(challenge), rank];
}
