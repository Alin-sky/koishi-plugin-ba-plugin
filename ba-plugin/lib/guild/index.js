...
async function gachaGroup(session, config) {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (CARD === 'ownerPB') {
            qunU = OWNER[koishi_1.Random.int(OWNER.length)];
            qunU.name = qunU.username;
        }
        else if (CARD === 'adminPB') {
            qunU = ADMIN[koishi_1.Random.int(ADMIN.length)];
            qunU.name = qunU.username;
        }
        else {
            qunU = MEMBER[koishi_1.Random.int(MEMBER.length)];
            qunU.name = qunU.username;
        }
        cards.push(qunU);
    }
    return await ps_1.gachaImage.result(cards, -1, MEMBER, ADMIN);
}
