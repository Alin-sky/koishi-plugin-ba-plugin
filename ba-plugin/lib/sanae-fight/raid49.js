...
exports.cut_twenty = exports.simplify = exports.raid49 = void 0;
...
function raid49() {
    // 导入已经建好的模型
    let mika = lodash_1.default.cloneDeep(models_1.mika_model);
    let maki = lodash_1.default.cloneDeep(models_2.maki_model);
    let akane = lodash_1.default.cloneDeep(models_3.akane_model);
    let ui = lodash_1.default.cloneDeep(models_4.ui_model);
    let ako = lodash_1.default.cloneDeep(models_5.ako_model);
    let himari = lodash_1.default.cloneDeep(models_6.himari_model);
    let binah = lodash_1.default.cloneDeep(models_7.binah_model);
    // 游戏主体
    // 游戏限制时长
    ...
    function attack(action) {
        if (action.receiver instanceof class_2.Boss && action.initiator instanceof class_1.Student) {
            let damage = action.content(action.initiator.status.atk);
            let damage_correct = (0, calculation_1.caused_dmg_correct)(action.initiator, action.receiver, Math.random(), Math.random(), Math.random());
    ...
    while (mika.status.hp_now > 0 && binah.status.hp_now > 0 && game_time <= total_time) {
        ...
        if (game_time >= mika.atk_time) {
            mika_atk_count += 1;
            action_list.push(new class_3.Action(mika, binah, "普通攻击", "damage", mika.attack.rate_func1, game_time, game_time + mika.attack.atk_duration, 0));
        ...
        if (mika_atk_count % 5 === 0 && mika_atk_count !== 0 && mika_basic_skill_on) {
            ...
            if (mika_basic_count % 3 === 0) {
                action_list.push(new class_3.Action(mika, binah, "强化版" + mika.skills[1].name, "damage", mika.skills[1].damage_rate2, game_time + mika.attack.atk_delay, game_time + mika.attack.atk_delay + mika.skills[1].duration, 0));
            }
            else {
                action_list.push(new class_3.Action(mika, binah, mika.skills[1].name, "damage", mika.skills[1].damage_rate1, game_time + mika.attack.atk_delay, game_time + mika.attack.atk_delay + mika.skills[1].duration, 0));
            }
        ...
        if (game_time >= maki.atk_time) {
            ...
            if (binah.buff.includes(maki.skills[1].debuff)) {
                action_list.push(new class_3.Action(maki, binah, "普通攻击", "damage", maki.attack.rate_func2, game_time, game_time + maki.attack.atk_duration, 0));
            }
            else {
                action_list.push(new class_3.Action(maki, binah, "普通攻击", "damage", maki.attack.rate_func1, game_time, game_time + maki.attack.atk_duration, 0));
            }
        ...
        if (game_time >= maki.basic_time) {
            action_list.push(new class_3.Action(maki, binah, maki.skills[1].name, "buff", maki.skills[1].debuff, game_time, game_time + maki.skills[1].duration, 15));
        ...
        if (game_time >= akane.atk_time) {
            akane_atk_count += 1;
            action_list.push(new class_3.Action(akane, binah, "普通攻击", "damage", akane.attack.rate_func1, game_time, game_time + akane.attack.atk_duration, 0));
            // akane的sub skill判定
            if (Math.random() <= 0.1 && akane_atk_count >= akane_sub_limit_count) {
                action_list.push(new class_3.Action(akane, binah, akane.skills[3].name, "buff", akane.skills[3].debuff, game_time, game_time + akane.skills[3].duration, 30));
        ...
        if (game_time >= akane.basic_time) {
            action_list.push(new class_3.Action(akane, binah, akane.skills[1].name, "damage", akane.skills[1].damage_rate1, game_time, game_time + akane.skills[1].duration, 0));
        ...
        if (game_time >= ui.atk_time) {
            ui_atk_count += 1;
            action_list.push(new class_3.Action(ui, binah, "普通攻击", "damage", ui.attack.rate_func1, game_time, game_time + ui.attack.atk_duration, 0));
        ...
        if (game_time >= ui.basic_time) {
            action_list.push(new class_3.Action(ui, mika, ui.skills[1].name, "buff", ui.skills[1].buff, game_time, game_time + ui.skills[1].duration, 33));
            action_list.push(new class_3.Action(ui, maki, ui.skills[1].name, "buff", ui.skills[1].buff, game_time, game_time + ui.skills[1].duration, 33));
            action_list.push(new class_3.Action(ui, akane, ui.skills[1].name, "buff", ui.skills[1].buff, game_time, game_time + ui.skills[1].duration, 33));
            action_list.push(new class_3.Action(ui, ui, ui.skills[1].name, "buff", ui.skills[1].buff, game_time, game_time + ui.skills[1].duration, 33));
        ...
        if (cost >= 6.4 + 0.2 * Math.random() && s1) {
            // ui ex
            cost -= 3;
            action_list.push(new class_3.Action(ui, mika, ui.skills[0].name, "buff", ui.skills[0].buff, game_time, game_time + ui.skills[0].duration, 33));
        ...
        if (cost >= 9.8 + 0.2 * Math.random() && s2 && !s1) {
            // ako ex
            cost -= 3;
            action_list.push(new class_3.Action(ako, mika, "「偵察データを共有します」", "buff", { "critical_multi": 0.395, "critical_dmg_multi": 0.733 }, game_time, game_time + 1.5, 16));
            s2 = false;
        }
        if (cost >= 8.5 + 0.1 * Math.random() && s3 && !s2) {
            // mika himari ex
            cost -= 6;
            action_list.push(new class_3.Action(mika, binah, mika.skills[0].name, "damage", mika.skills[0].damage_rate1, game_time, game_time + mika.skills[0].duration, 0));
            action_list.push(new class_3.Action(himari, mika, "実力をお見せしましょう」", "buff", { "atk_multi": 1.05 }, game_time, game_time + 1.5, 15.47));
        ...
        if (cost >= 2 && s4 && !s3) {
            // akane ex
            cost -= 2;
            action_list.push(new class_3.Action(akane, binah, akane.skills[0].name, "damage", akane.skills[0].damage_rate1, game_time, game_time + akane.skills[0].duration, 0));
            action_list.push(new class_3.Action(akane, binah, akane.skills[0].name, "buff", akane.skills[0].debuff, game_time, game_time + akane.skills[0].duration, 30));
...
// 定义支持内容文字分割的函数
function cut_twenty(notebook) {
    let notebook_copy = notebook;
