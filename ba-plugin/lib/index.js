...
exports.schema = koishi_1.Schema.object({
    JApersona: koishi_1.Schema.string().default('实莉').description('日服UP池角色'),
...
function apply(ctx, config) {
    ...
    ctx.on('ready', () => {
        ...
        if ((test_1.cardPool.threeStar).some(a => a.name === config.JApersona)
            || (test_1.cardPool.limit2).some(a => a.name === config.JApersona)) { //同时检测日服常驻和限定角色
        ...
        if ((test_1.cardPool.threeStar).some(a => a.name === config.INpersona)
            || (test_1.cardPool.limit2).some(a => a.name === config.JApersona)) { //同时检测急服常驻和限定角色
    ...
    ctx.command("ba", '碧蓝档案抽卡模拟器')
        ...
        if ((args[0]) == null) {
            let a = "BlueArchive抽卡模拟器\n" +
                ...
                "三星的概率是2.3%\n" +
                "up角的概率是0.7%";
            return (0, jsx_runtime_1.jsx)("message", { forward: true, children: (0, jsx_runtime_1.jsxs)("message", { children: [(0, jsx_runtime_1.jsx)("author", { "user-id": session.selfId, nickname: session.selfId, avatar: "url" }), a, (0, jsx_runtime_1.jsxs)("message", { children: [(0, jsx_runtime_1.jsx)("author", { "user-id": session.selfId, nickname: session.selfId, avatar: "url" }), "\u89D2\u8272\u5BF9\u5E94\u540D\u79F0\u56FE", (0, jsx_runtime_1.jsx)("image", { url: 'http://124.221.99.85:8088/name.jpg' })] })] }) });
        }
        else {
            //急服名称检测
            if ((args[0]) === 'iup') {
                if (test_1.cardPool.limit.some(e => e.name === args[1])) {
                    exports.inup = '爱丽丝';
                    return '啊哈哈...国急服还没实装呢';
                }
                else if ((test_1.cardPool.threeStar).some(e => e.name === args[1])
                    || (test_1.cardPool.limit3).some(e => e.name === args[1])) {
                    exports.inup = (args[1]);
            ...
            if ((args[0]) === 'jup') {
                if ((test_1.cardPool.threeStar).some(e => e.name === args[1]) ||
                    (test_1.cardPool.limit2).some(e => e.name === args[1])) {
                    exports.jaup = (args[1]);
                    ...
                    exports.jaup = (args[1]);
                    return '整活成功，已设置日服up角色为 ' + exports.jaup;
                }
                else {
                    exports.jaup = '实莉';
                    return '啊哈哈...角色名字不对喵';
            ...
            ;
            if ((args[0]) === 'name') {
                return "角色对应名称图" + (0, jsx_runtime_1.jsx)("image", { url: 'http://124.221.99.85:8088/name.jpg' });
            }
        }
        {
            return '呜呜...输入错误了';
    ...
    ctx.command('攻略', "bawiki的攻略图和角色评分")
        ...
        if ((args[0]) == null) {
            ...
            if ((args[0]) === '随机表情' || (args[0]) === '抽表情') {
                ...
                return (0, jsx_runtime_1.jsx)("image", { url: uurl + 'emoji/' + 'Clanchat_Emoji_' + random1 + '_Tw' + '.png' });
            }
            if ((args[0]) === '随机漫画' || (args[0]) === '抽漫画') {
                let random = Math.floor(Math.random() * 218) + 1;
                let random1 = random.toString().padStart(4, '1');
                return (0, jsx_runtime_1.jsx)("image", { url: alincloud + 'comic/' + random1 + '.jpg' });
            }
            ...
            for (const obj of data_1.data) {
                if (obj.name === args[0]) {
                    return (0, jsx_runtime_1.jsx)("image", { url: uurl + 'student/' + obj.link[0] + '.png' });
                }
            }
            for (const obj of data_1.data) {
                if (obj.name != args[0]) {
