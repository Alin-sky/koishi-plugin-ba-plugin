...
async function stuCardmaker(card, RPool, SRPool) {
    if (RPool.some((item: { name: any; }) => item.name === card)) {
        const CARDSTAR1 = await Jimp.read(alincloud+'stuimg'+'/assets/☆card.png')
        const CARDSTAR2 = await Jimp.read(alincloud+'stuimg'+'/assets/card☆1.png')
        return await cardmaker(CARDSTAR1, CARDSTAR2, card)
    } else if (SRPool.some((item: { name: any; }) => item.name === card)) {
        const CARDSTAR1 = await Jimp.read(alincloud+'stuimg'+'/assets/☆☆card.png')
        const CARDSTAR2 = await Jimp.read(alincloud+'stuimg'+'/assets/card☆2.png')
        return await cardmaker(CARDSTAR1, CARDSTAR2, card)
    } else {
        const CARDSTAR1 = await Jimp.read(alincloud+'stuimg'+'/assets/☆☆☆card.png')
        const CARDSTAR2 = await Jimp.read(alincloud+'stuimg'+'/assets/card☆3.png')
        return await cardmaker(CARDSTAR1, CARDSTAR2, card)
    }
    //头像处理
    async function cardmaker(CARDSTAR1, CARDSTAR2, card) {
        let cache = new Map()
        const CARDSTAR3 = await Jimp.read(alincloud+'stuimg'+'/assets/Student/' + card + '.png')
        const CARDSTAR4 = await Jimp.read(alincloud+'stuimg'+'/assets/margin.png')
...
export async function gachaImageOne(_temporary, cardArray, stat, RPool, SRPool) {
    ...
    let gachaBG = await Jimp.read(path.resolve(alincloud+'stuimg'+'assets/gachaBG.png'))
...
export async function gachaImageTen(_temporary, cardArray, stat, RPool, SRPool) {
    ...
    const FONT = await Jimp.loadFont(path.resolve(__dirname, '../font/ArialBlack.fnt'))
    const FONT2 = await Jimp.loadFont(path.resolve(__dirname, '../font/BA2.fnt'))
    let gachaBG = await Jimp.read(alincloud+'stuimg'+'/assets/gachaBG.png')
    let tempCard = await Jimp.read(alincloud+'stuimg'+'/assets/☆card.png')
