...
import {
  日服井, 国际服井, 国际服十连,
  日服十连, turnout, output, clearOutput,
  count3pro, count1, output101,
  cleardata, output102, count2, newcard, count3,
} from './data/test';
import { url } from "inspector";
import { secureHeapUsed } from "crypto";
import { builtinModules } from 'module';
import { constrainedMemory } from 'process';
import { cardname, output103, nocai } from './data/test';
          ...
          {output.map((item) => (
            <message>
              ...
              <image url={pathToFileURL(resolve
                (__dirname, './data/photo/' + item + '.png')).href} />
            </message>
          ), clearOutput()
          )}
        </message>
        ...
        return <message forward>
          ...
          {output101.map((out101) => (
            <message>
              ⭐{out101}
              <image url={pathToFileURL(resolve
                (__dirname, './data/photo/' + out101 + '.png')).href} />
            </message>
          )
          )}
          ...
          {output102.map((out102) => (
            <message>
              ⭐⭐{out102}
              <image url={pathToFileURL(resolve
                (__dirname, './data/photo/' + out102 + '.png')).href} />
            </message>
          )
          )}
            ...
            {output103.map((out103) => (
              <message>
                ⭐⭐⭐{out103}
                <image url={pathToFileURL(resolve
                  (__dirname, './data/photo/'+ out103 +'.png')).href} />
              </message>
            ))
          }
          {cleardata()}
