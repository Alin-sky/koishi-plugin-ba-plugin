...
export const schema = Schema.object({
  JApersona: Schema.string().default('实莉').description('日服UP池角色'),
  INpersona: Schema.string().default("爱丽丝").description('国急服UP池角色'),
  server: Schema.union([
...
export function apply(ctx: Context, config: Config,) {
  ...
  ctx.on('ready', () => {
    //up角色初始化以及防整活
    if ((cardPool.threeStar).some(a => a.name === config.JApersona)
      || (cardPool.limit2).some(a => a.name === config.JApersona)) {//同时检测日服常驻和限定角色
      jaup = config.JApersona
      console.log('已设置日服up池角色' + jaup)
    } else {
      jaup = '实莉'
      console.log('已强行设置日服up池角色' + jaup)

    }


    //急服
    if ((cardPool.threeStar).some(a => a.name === config.INpersona)
      || (cardPool.limit2).some(a => a.name === config.JApersona)) {//同时检测急服常驻和限定角色
      inup = config.INpersona
      console.log('已设置国际服up池角色' + inup)
    } else {
      inup = '爱丽丝'
      console.log('已强行设置国际服up池角色' + inup)
    }
    ...
    .action(async ({ session }, ...args) => {
      if ((args[0]) == null) {
        ...
        if ((args[0]) === '随机表情' || (args[0]) === '抽表情') {
          ...
          return <image url={uurl + 'emoji/' + 'Clanchat_Emoji_' + random1 + '_Tw' + '.png'} />
        }
        ...
        if ((args[0]) === '随机漫画' || (args[0]) === '抽漫画') {
          ...
          return <image url={alincloud + 'comic/'+ random1+'.jpg'} />
        }
        ...
        for (const obj of data) {
          if (obj.name === args[0]) {
            return <image url={uurl + 'student/' + obj.link[0] + '.png'} />
          }
        }
        for (const obj of data) {
          if (obj.name != args[0]) {
