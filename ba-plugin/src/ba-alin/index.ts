      ...
      .action(async ({ session }, ...args) => {
        ...
        if ((args[0]) == null) {
          ...
          if ((args[0]) === '千里眼' || (args[0]) === '国际服千里眼') {
            return (h('image', { url: "https://cdnimg.gamekee.com/wiki2.0/images/w_2500/h_5336/829/43637/2023/6/10/585780.png" }))
          }
          /*
          function old (){
        ...
        */
          ...
          if (outstu.status == 200) {
            ...
            if (!obj) { session.send('呜呜，等待超时，请重新触发指令') }
            //匹配返图
            if (['1', '2', '3', '4'].includes(obj)) {
              ...
              function datafs() {
                ...
                let newpath = (path.resolve(__dirname) + "\\" + outstu1.data[0].hash + '.png')
                fs.rename(oldpath, newpath, (err) => {
                  if (err) {
                    console.error(err);
                  } else {
