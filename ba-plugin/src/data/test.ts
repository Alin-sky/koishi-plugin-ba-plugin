...
drawResult.threeStar.forEach((card) => {
  output += '⭐⭐⭐'
   ...
   +':'
   + (h('image',
   {url:(pathToFileURL(resolve(__dirname+'/photo/'+card.name+'.png')).href)}));
...
for (let i = 0; i < 10; i++) {
  ...
  if (rand < probabilities.oneStar) {
    ...
    drawResult.oneStar.push(card);
    count1++
    if (pityCounter >= 9 && count2 === 0 && count3 === 0) {
      // 如果已经连续九次没有抽到二星或三星卡片，则保底
      const index = drawResult.oneStar.findIndex(c => c.name !== card.name);
      if (index !== -1) {
        // 找到一个不是当前抽到的卡片的一星卡片，将其替换为一个二星卡片
        drawResult.oneStar.splice(index, 1, cardPool.twoStar[Math.floor(Math.random() * 22)]);
        count1--;
        count2++;
        pityCounter = 0;
      }
