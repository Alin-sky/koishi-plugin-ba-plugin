...
export const usage = "## 角色测评数据来源于<a href=\"https://ba.gamekee.com/\">bawiki</a>\n"+
...
// 创建星级类，用于接入学生类
    // 星级属性是一个字典，用于接入星级类，例：const stat_status = {星级: [hp_max, atk, healing], ...}
    interface Star_status {
      [key: number]: number[]
    }
      // 星级类
    class Stars {
      ...
      constructor(stars: string, status: Star_status, basic_stars: number = 3) {
        this.stars = stars;
        this.status = status;
    ...
    class Relationship {
    relationship: number;
    bonus_func: [number, number, number, number];
  
    constructor(relationship: number, bonus_func: [number, number, number, number]) {
    ...
    class Status {
      ...
      // 用于更新buff的函数
      update_buff_status(buff: Record<string, number>[]) {
        let dict_sums: Record<string, number> = {};
    ...
    // 创建装备类，用于接入学生类
      // 装备属性是一个字典，用于接入装备类，例: const equipment_status = {属性1,:数值1,  ...}
    ...
    class Equipment{
      ...
      constructor(form: string, level: number, status: Equipment_status){
        this.form = form;
        this.level = level;
        this.status = status;
      }
    ...
    class Attack {
      ...
      constructor(rate: number, atk_duration: number, atk_delay: number, reload_duration: number,
        reload_start_delay: number, reload_end_delay: number, reload_count: number) {
        this.rate = rate;
    ...
    // 创建技能类，用于接入学生类
      // buff与debuff是字典
    interface Skill_buff {
      [key: string]: number
    }
    interface Skill_debuff {
      [key: string]: number
    }
     // 技能类
    class Skill {
    ...
    interface Student_interface {
      ...
      status: Status;
      equipments: [Equipment, Equipment, Equipment, Equipment];
      attack: Attack;
      skills: [Skill, Skill, Skill, Skill];
      buff?: Skill_buff[] | Skill_debuff[];
      dmg_rate?: number;
    ...
    class Student implements Student_interface {
      constructor(
          ...
          public equipments: [Equipment, Equipment, Equipment, Equipment],
          public attack: Attack,
          public skills: [Skill, Skill, Skill, Skill],
          public buff: Skill_buff[] | Skill_debuff[] = [],
          public dmg_rate: number = 1,
          public position: [number, number, number] = [0, 0, 0],
          public atk_time: number = 0,
          public ex_time: number = 0,
          public basic_time: number = 0,
          public passive_time: number = 0,
          public sub_time: number = 0,
      ) {}
    ...
    class Boss implements Boss_interface {
      constructor(
        public name: string,
        public difficulty: string,
        public status: Boss_status,
        public attack: Boss_attack,
        public skills: any,
        public buff: Skill_buff[] | Skill_debuff[] = [],
        public dmg_rate: number = 1,
        public position: [number, number, number] = [0, 0, 0],
    ...
    class Action implements Action_interface {
      constructor(
        public initiator: Student | Boss,
        public receiver: Student | Boss,
        public name: string,
        public id: string,
        public content: any = {},
        public act_time: number,
        public effect_time: number,
  ...
  // 伤害计算函数
  // 地形补正
  function land_form_correct(adaption: string): number {
    if (adaption === "SS"){
      return 1.3;
    } else if (adaption === "S"){
      return 1.2;
  ...
  // 装甲补正
  function armor_correct(atk_type: string, def_type: string): number {
    if (atk_type === "red") {
      if (def_type === "red") {
        return 2;
  ...
  // 等级差补正
  function level_correct(atk_lv: number, def_lv: number): number {
    if (atk_lv >= def_lv) {
      return 1;
    } else {
  ...
  // 安定浮动补正
  function stability_correct(stability: number, face: number): number {
  ...
  // 防御力减伤补正
  function def_correct(defence: number): number {
    return 1 - defence / (defence + 1666.66);
  } 
  // 暴击率与暴击伤害补正
  function critical_correct(atk_critical: number, atk_critical_dmg: number, def_critical_res: number, 
  ...
  // boss自带减伤率修正
  function boss_dmg_res_correct(boss_dmg_res: number): number {
    return 1 - boss_dmg_res
  }
  // 命中率修正
  function accuracy_correct(accuracy: number, evasion: number, face: number): number {
  ...
  // 总伤害修正
  function caused_dmg_correct(attacker: Student | Boss, defender: Boss, stability_face: number, critical_face: number, 
    accuracy_face: number): number {
      ...
      let attacker_arm_correct_rate: number = armor_correct(attacker.status.atk_type, defender.status.def_type);
      // 特攻修正
  ...
  const mika_status = new Status(32833, 2610, 115, 4505, 111, 1417, 25, 100, 2, 0.5, 1376, 450, 100, 100, 1, 5, "yellow",
    "red", {"street": "D", "outdoor": "A", "indoor": "S", "exclusive3": {"indoor": "SS"}}, []);
  ...
  const mika_equipments: [Equipment, Equipment, Equipment, Equipment] = [hat_t7, badge_t7, watch_t7, mika_exclusive_weapon];
  const mika_attack = new Attack(1, 0.77, 1.75, 2.2, 0.67, 0.53, 5);
  const mika_ex_skill = new Skill("Kyrie Eleison", 6, 15.4, {}, {}, 4.25, 2, 
    "该技能会按敌人血量百分比增加同等百分比伤害");
  const mika_basic_skill = new Skill("星の呼び声", 0, 1.69, {}, {}, 0.75, 1.5,
    "每5次普攻发动1次，每发动3次召唤陨石伤害更高，取消射击动作的后摇，一般发动时间与换弹时间一致");
  const mika_passive_skill = new Skill("Gloria Patri", 0, 0, {"atk_multi": 0.266}, {}, 66666, 0,
    "被动技能，开局发动，加百分比攻击力和贯通克制倍率");
  ...
  const maki_status = new Status(19243, 2387, 426, 4360, 98, 196, 245, 100, 2, 0.5, 1424, 750, 100, 100, 1, 15, "yellow",
    "red", {"street": "C", "outdoor": "S", "indoor": "C", "exclusive3": {"outdoor": "SS"}}, []);
  ...
  const maki_equipments: [Equipment, Equipment, Equipment, Equipment] = [gloves_t7, hairpin_t7, watch_t7, maki_exclusive_weapon];
  ...
  const maki_basic_skill = new Skill("くらえ、ペイントボール！", 0, 0, {}, {"defence_multi": -0.348, "marked_by_maki": 15}, 1.5, 1, 
    "每25s发动一次，标记并减防1名敌人，持续15s");
  const maki_passive_skill = new Skill("芸術的な演出", 0, 0, {"atk_speed_multi": 0.266}, {}, 66666, 0, 
    "被动技能，开局发动，加攻速");
  const maki_sub_skill = new Skill("芸術の苦しみ", 0, 0, {"dmg_add": 0.283}, {}, 66666, 0, 
    "只对标记单位生效的追伤，与攻击段数有关");
  ...
  const akane_status = new Status(23151, 934, 102, 4133, 101, 1119, 203, 100, 2, 0.5, 980, 550, 100, 100, 1, 6, "yellow",
    "red", {"street": "A", "outdoor": "A", "indoor": "D", "exclusive3": {"indoor": "B"}}, []);
  ...
  const akane_equipments: [Equipment, Equipment, Equipment, Equipment] = [boots_t7, hairpin_t7, necklace_t7, akane_exclusive_weapon];
  ...
  const akane_passive_skill = new Skill("速やかに前進します", 0, 0, {"move_speed_multi": 0.266}, {}, 66666, 0, 
    "开局发动，加移速");
  const akane_sub_skill = new Skill("的確に制圧します", 0, 0, {}, {"evasion_multi": -0.127}, 0.67, 0, 
    "10%概率普攻触发，持续30s，cd5s");
  const akane_skills: [Skill, Skill, Skill, Skill] = [akane_ex_skill, akane_basic_skill, akane_passive_skill, akane_sub_skill];
  const akane = new Student("茜", akane_stars, 85, akane_relationship, akane_status, akane_equipments, akane_attack,
                  akane_skills);
  ...
  const ui_status = new Status(18050, 2516, 100, 4639, 941, 190, 209, 100, 2, 0.5, 1908, 750, 100, 100, 1, 7, "red",
    "red", {"street": "B", "outdoor": "S", "indoor": "D", "exclusive3": {"outdoor": "SS"}}, []);
  ...
  const ui_equipments: [Equipment, Equipment, Equipment, Equipment] = [gloves_t7, hairpin_t7, amulet_t7, ui_exclusive_weapon];
  const ui_attack = new Attack(1, 1.4, 1.33, 2.47, 0.5, 0.53, 7);
  const ui_ex_skill = new Skill("古書の専門家", 3, 0, {"cost_multi": 0.5, "atk_multi": 0.161}, {}, 2, 0, 
    "2次减费50%，加攻46s");
  const ui_basic_skill = new Skill("伝授されていく知識", 0, 0, {"critical_multi": 0.259}, {}, 1.5, 0, 
    "每40s发动一次群体加暴击，持续33s");
  const ui_passive_skill = new Skill("専門家の知見", 0, 0, {"critical_multi": 0.266}, {}, 66666, 0, 
    "开局发动，加自己暴击");
  const ui_sub_skill = new Skill("神経過敏", 0, 0, {"atk_speed_multi": 0.459}, {}, 0, 0, 
    "ex使用时触发，持续20s加自己攻速");
  const ui_skills: [Skill, Skill, Skill, Skill] = [ui_ex_skill, ui_basic_skill, ui_passive_skill, ui_sub_skill];
  const ui = new Student("忧", ui_stars, 85, ui_relationship, ui_status, ui_equipments, ui_attack,
               ui_skills);
  ui.atk_time = 5.5;
  ui.basic_time = 41.8;
  
  // 建立简单的亚子与日鞠模型,部分参数置零或用ui模型中参数替代，该部分不参与任何计算，仅作为占位
  const placeholder1: Star_status = {0: [0,0,0]};
  const placeholder2 = new Relationship(0, [0, 0, 0, 0]);
  const placeholder3 = new Status(18050, 2516, 100, 4639, 941, 190, 209, 100, 2, 0.5, 1908, 750, 100, 100, 1, 7, "red",
  "red", {"street": "B", "outdoor": "S", "indoor": "D", "exclusive3": {"outdoor": "SS"}}, []);
  const placeholder4 : [Equipment, Equipment, Equipment, Equipment] = [gloves_t7, hairpin_t7, amulet_t7, ui_exclusive_weapon];
  ...
  while(mika.status.hp_now > 0 && binah.status.hp_now > 0 && game_time <= total_time){
    ...
    if (mika_atk_count % 5 === 0 && mika_atk_count !== 0 && mika_basic_skill_on) {
      ...
      if (mika_basic_count % 3 === 0) {
        action_list.push(new Action(mika, binah, "强化版" + mika.skills[1].name, "damage", mika_basic2, 
          game_time + mika.attack.atk_delay, game_time + mika.attack.atk_delay + mika.skills[1].duration, 0));
      } else {
        action_list.push(new Action(mika, binah, mika.skills[1].name, "damage", mika_basic1, 
          game_time + mika.attack.atk_delay, game_time + mika.attack.atk_delay + mika.skills[1].duration, 0));
      }
    ...
    if (cost >= 8.5 + 0.1 * Math.random() && s3 && !s2){
      // mika himari ex
      cost -= 6;
      action_list.push(new Action(mika, binah, mika.skills[0].name, "damage", mika_ex, game_time,
    ...
    if (cost >= 2 && s4 && !s3){
      // akane ex
      cost -= 2;
      action_list.push(new Action(akane, binah, akane.skills[0].name, "damage", akane_ex, game_time,
...
function simplify(notebook : any[]): any[] {
  ...
  for (let oo of jilu)
    if ( (args[0])  === oo ){
      if (latest.length > 0) {
        ...
        ("message", { forward: true, children: 
          (jsx_runtime_1.jsxs)("message",
           { children: [(jsx_runtime_1.jsx)
            ("author", { "user-id": session.selfId }), 
            (jsx_runtime_1.jsx)("p", { children: latest })] }) });
...
//看到这里的别骂了，阿林英语答辩，不会写好的对象名，一定会改的
      let x1 = ['1','打大蛇','大蛇',"大蛇打一次","打一次大蛇",'大蛇凹一次']
      for (let y1 of x1 )
    if((args[0]) == y1 ){
      let result = raid49();
...
.action(async({session},...args) =>{
  ...
  .action(() =>{
    return (h('image',
    {url:(pathToFileURL(resolve(__dirname+'/data/photo1/n2.jpg')))}))
 } )
 
  ctx.command("家具",'学生互动家具查询')
  .action(() =>{
    return (h('image',
    {url:(pathToFileURL(resolve(__dirname+'/data/photo1/n1.jpg')))}))
 } )
